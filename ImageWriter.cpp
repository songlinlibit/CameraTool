...
bool ImageWriter::WriteTiff(  const tFrameInfo & frame_info, const char* filename)
{
    bool result = true;
    ...
    if( NULL != TiffFile )
    {
        ...
        if( true == result)
        {
            ...
            if( !result)
            {
                    qDebug()<<"error could not write frame buffer";
            }
        }
        else
        {
            qDebug()<<"error could not write TIFF standard flags";
        }
        (*TiffLibrary::Get().TiffClose)(TiffFile);
    }
    else
    {
        qDebug()<<"error could not open tiff file for writing";
        result = false;
    }
    return result;
}
